...
struct svm_problem *extract_model(char *path, svm_parameter *param, svm_node *x_space) {
...
svm_parameter * get_param_from_cmd(int argc, char ** argv)
{
	svm_parameter *param = (svm_parameter *)malloc(sizeof(struct svm_parameter));
	param->svm_type = C_SVC;
	param->kernel_type = RBF;
	param->degree = 3;
	param->gamma = 0;
...
{
	const char *normalize_def = "normalized.txt";
	const char *model_def = "normalized.model";
	char *normalize;
	char *model;
	const char *err_msg;
	...
	normalize = (char *)malloc(sizeof(char) * (len_path + 14));
	model = (char *)malloc(sizeof(char) * (len_path + 16));
	strcpy(normalize, path);
	strcat(normalize, normalize_def);
	strcpy(model, path);
	strcat(model, model_def);
	svm_parameter *param = get_param_from_cmd(argc, argv);
	svm_problem *prob = extract_model(normalize, param, x_space);
	err_msg = svm_check_parameter(prob, param);
	if (err_msg) {
	...
	if (param->crossvalidation) {
		...
		if (svm_save_model(model, model_train)) {
...
void do_cross_validation(svm_problem * prob, svm_parameter * param)
{
	int i;
	int total_correct = 0;
...
double predict_test(SAMPLE * audio_signal, char * path, int predict_probability)
{
	...
	if (predict_probability) {
		if (svm_type == NU_SVR || svm_type == EPSILON_SVR) {
			...
			prob_estimate = (double *)malloc(sizeof(double) * nr_class);
			printf("labels : ");
			for (int i = 0; i < nr_class; ++i) {
				printf("%d ", labels[i]);
			}
	...
	node = (svm_node *)realloc(node, sizeof(struct svm_node)*(FEATSIZE + 1));
	node[FEATSIZE].index = -1;
	/*for (int i = 0; i < FEATSIZE + 1; ++i) {
		printf("%d : %f \n", node[i].index, node[i].value);
	}*/
	
	double predict_label;
	if (predict_probability && (svm_type == C_SVC || svm_type == NU_SVC)) {
		predict_label = svm_predict_probability(model, node, prob_estimate);
		printf("%g", predict_label);
		for (int i = 0; i < nr_class; ++i) {
			printf(" %g", prob_estimate[i]);
		}
		printf("\n");
	}
	else {
...
svm_node * build_node_from_signal(SAMPLE * audio_signal, char *path)
{
	...
	for (int i = 0; i < FEATSIZE; ++i) {
		node[i].index = i+1;
		node[i].value = normalize_detect[i];
	}
